...
from Grounded_SAM_2.TrackingAPI_before import SAM2VideoSegmentation as GDSAM2Processor
...
@timed("初始化GDSAM2")
def create_gdsam2_processor(model_path, camera_info):
    return G(model_path, camera_info)
...
@timed("GDSAM2切割")
def gdsam2_process_frame(processor, frame_image, frame_idx, save_dir):
    return processor.run(frame_image, frame_idx, save_dir)
...
def capture_and_process(cad_path, sam_tmp_dir, data_dir, track_vis_dir, track_pose_dir, segmentor_model, stability_score_thresh, use_parallel_mode, cup):
    ...
    while frame_count < num_rgb:
        ...
        if frame_count == 0:    # 仅对首帧进行处理
            sam_process_frame(processor=sam_processor, rgb_path=rgb_path, depth_path=depth_path, mask_path=os.path.join(data_dir, "first_frame_mask.png"), cam_info=cam_info)  # 将读取到的相机参数传输入sam6d的分割文件
