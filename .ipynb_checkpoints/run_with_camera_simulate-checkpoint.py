...
from sam2.build_sam import initialize, compose, instantiate
...
@timed("GDSAM2切割")
def gdsam2_process_frame(ref_mask_path, ref_image_path, processor, intput_image_path, output_mask_path):
    return processor.process_frames(ref_mask_path, ref_image_path, intput_image_path, output_mask_path)
...
def capture_and_process(cad_path, sam_tmp_dir, data_dir, track_vis_dir, track_pose_dir, segmentor_model, stability_score_thresh, use_parallel_mode, cup):
    # 相机读取的目录
    ...
    camera_dir = 'simulate_captured'  # 保存捕捉到的图像的文件夹
    sam2_checkpoint = "Grounded_SAM_2/checkpoints/sam2.1_hiera_large.pt" # SAM2权重
    # model_cfg = "configs/sam2.1/sam2.1_hiera_l.yaml" 
    model_cfg = "sam2.1_hiera_l.yaml" 

    ref_mask_path = "simulate_captured/ref_mask.png"
    ref_image_path = "simulate_captured/ref_image.png"
    track_vis_dir = "simulate_captured/track_vis"
    ...
    while frame_count < 300:
        ...
        if frame_count == 0:    # 仅对首帧进行处理
            sam_process_frame(processor=sam_processor, rgb_path=ref_mask_path, depth_path=depth_path, mask_path=ref_mask_path, cam_info=cam_info)  # 将读取到的相机参数传输入sam6d的分割文件
            del sam_processor   # 工作完成，释放显存


        # ************用GD SAM2进行checking************ #
        gdsam2_process_frame(gdsam2_processor, ref_mask_path=ref_mask_path, ref_image_path=ref_mask_path, intput_image_path=rgb_path, output_mask_path=mask_path)
        ...
        print(f"检测帧{frame_count}.png已保存到{save_path}", end="   ")
