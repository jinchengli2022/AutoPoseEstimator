...
from Pnt2MP4 import png_to_video
...
def capture_and_process(sam2_checkpoint, model_cfg, cad_path, sam_tmp_dir, data_dir, track_vis_dir, mask_vis_dir,
                        track_pose_path, segmentor_model, stability_score_thresh, text_prompt, use_ref, rounds,
                        init_flag):
    # 制定相机临时存放地
    ...
    while frame_count < 1:
        ...
        if frame_count == 0:  # 仅对首帧进行处理
            if use_ref:
                ...
                # gdsam2_processor.predict_mask(rgb_path=rgb_path, mask_path=ref_mask_path, text_prompt=text_prompt)
                sam_process_frame(processor=sam_processor, rgb_path=rgb_path, depth_path=depth_path,
                                  mask_path=ref_mask_path, cam_info=cam_info)  # 将读取到的相机参数传输入sam6d的分割文件
                print("首帧切割完成")
...
if __name__ == "__main__":
    ...
    if args.data_dir is None:
        args.data_dir = f"Data/real_data/ism_test/episode_1"
    if args.track_vis_dir is None:
        args.track_vis_dir = f"tmp"
    if args.mask_vis_dir is None:
        args.mask_vis_dir = f"tmp"
    if args.track_pose_path is None:
        args.track_pose_path = f"tmp"
...
#         parser = argparse.ArgumentParser()
#         # input mesh
#         parser.add_argument("--cad_path", default=f"Data/real_data/{name}_mesh/{name}.obj", help="Path to CAD model")  # 注意输入单位为mm，中间处理为m
#         # 参量输入
...
#         parser.add_argument("--sam_tmp_dir", default=f"Data/real_data/{name}_mesh/{name}_tmp", help="tmp directory")
#         # 参数配置
...
#         # parser.add_argument("--use", type=float, default=0.97, help="SAM stability score threshold")

#         # 数据输入
#         parser.add_argument("--data_dir", default=f"Data/data_real/{task}/episode_{time}", help="rgb, depth and mask")
#         # 数据输出
#         parser.add_argument("--track_vis_dir", default=f"output/data_real/{task}/episode_{time}/{name}{id}/{name}{id}_track_vis", help="rgb, depth and mask")
#         parser.add_argument("--mask_vis_dir", default=f"output/data_real/{task}/{name}{id}/{name}{id}_mask_vis", help="rgb, depth and mask")
#         parser.add_argument("--track_pose_path", default=f"output/data_real/{task}/episode_{time}/", help="rgb, depth and mask")

#         # parser.add_argument("--track_vis_dir", default=f"output/{task}/episode_{time}/{name}{id}/{name}{id}_track_vis", help="rgb, depth and mask")
#         # parser.add_argument("--mask_vis_dir", default=f"output/{task}/{name}{id}/{name}{id}_mask_vis", help="rgb, depth and mask")
#         # parser.add_argument("--track_pose_path", default=f"output/{task}/episode_{time}/", help="rgb, depth and mask")
