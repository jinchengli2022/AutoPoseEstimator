...
class FoundationPose:
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5, index=0):
    ...
    # 检查 depth 和 ob_mask 的形状是否一致
    if depth.shape != ob_mask.shape:
        print(f"Depth shape: {depth.shape}, Mask shape: {ob_mask.shape}")
        raise ValueError("Depth and mask shapes do not match!")
    ...
    if self.debug>=2:
      ...
      pcd = toOpen3dCloud(xyz_map[valid], rgb[valid])   # error here
