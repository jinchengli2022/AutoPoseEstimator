...
def capture_and_process(sam2_checkpoint, model_cfg, cad_path, sam_tmp_dir, data_dir, track_vis_dir, mask_vis_dir, track_pose_path, segmentor_model, stability_score_thresh, use_parallel_mode, rounds, text_prompt):
    # 制定相机临时存放地
    camera_dir = f'captured/{name}/'
    # 删除旧目录并重新创建
    ...
    # 读取相机内参
    camera_path = os.path.join(camera_dir, 'scene_camera.json')
...
[[-0.01710877,  0.79030309, -0.61247583, -0.07469574],
 [ 0.51127073,  0.53333898,  0.67390744,  0.01720717],
 [ 0.8592497 , -0.30161155, -0.41318389,  0.70181402],
 [ 0.        ,  0.        ,  0.        ,  1.        ]]
        )
        ...
        relevant_pose = np.linalg.inv(T_cam_ref_init) @ pose
        msg = Matrix4x4Flat()
        msg.matrix = relevant_pose.flatten().tolist()  # 将矩阵数据转化为列表
        lc.publish("relevant_pose", msg.encode())
        print("Broadcasting matrix")

        # 可视化保存
        vis_img = fp_processor.visualize_result(color_img, pose, fp_processor.config['camera_matrix'])
        cv2.imshow("Preview", vis_img[..., ::-1]) 
        ...
        # np.save(os.path.join(track_pose_path, "pose.npy"), np.array(pose))
        frame_count += 1
