...
### 中文
## 介绍
**AutoPoseEstimator** 基于Foundationpose可自动化实现任意物体的全流程的实时 6DoF 位姿追踪。通过读取外部 **mesh** 文件，经由PBR过程，支持**ISM**，**Grounding SAM2**, **Qwen-VL**三种策略生成初始帧**mask**，由**Foundationpose** 进行目标姿态估计，支持**高精定位**(稳定度高，适合预采数据)及**实时定位**（实时性好，适合实机部署）两种模式。

## 主要功能

- **Mesh 导入**：读取外部的 3D 模型（mesh），经由PBR过程自动导出参考视角图。
- **初始帧 生成**：使用 **Grounded SAM 2** 自动为输入图像生成 **mask**，无需手动标注。
