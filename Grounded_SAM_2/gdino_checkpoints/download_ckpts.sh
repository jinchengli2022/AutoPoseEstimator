...
wget --no-check-certificate $swint_ogc_url || { echo "Failed to download checkpoint from $swint_ogc_url"; exit 1; }

echo "Downloading groundingdino_swinb_cogcoor.pth checkpoint..."
wget --no-check-certificate $swinb_cogcoor_url || { echo "Failed to download checkpoint from $swinb_cogcoor_url"; exit 1; }
