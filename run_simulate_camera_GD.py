...
# # 在函数外声明全局变量
# fp_processor = None
# gdsam2_processor = None

def capture_and_process(sam2_checkpoint, model_cfg, cad_path, sam_tmp_dir, data_dir, track_vis_dir, mask_vis_dir, track_pose_path, segmentor_model, stability_score_thresh, text_prompt, use_ref, rounds, init_flag):
    # 制定相机临时存放地
    ...
    while frame_count < frame_num:
        ...
        if frame_count == 0:    # 仅对首帧进行处理
            if use_ref:
                ...
                # sam_process_frame(processor=sam_processor, rgb_path=rgb_path, depth_path=depth_path, mask_path=ref_mask_path, cam_info=cam_info)  # 将读取到的相机参数传输入sam6d的分割文件
                print("首帧切割完成")
    ...
    # 将图片变成png
    png_to_video(track_vis_dir, f"{track_vis_dir}/../../track_vis_{name}.mp4", 15)
